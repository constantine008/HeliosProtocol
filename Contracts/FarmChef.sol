...
contract MasterChef is Ownable {
    ...
    function getTimeMultiplex(uint256 sec_) public pure returns (uint256) {
        ...
        else if (sec_ <= 1 days) return 101;
        else if (sec_ <= 5 days) return 108;
        else if (sec_ <= 15 days) return 120;
        else return 120;
    }

    // Deposit LP tokens to MasterChef for SUSHI allocation.
    function deposit(uint256 _pid, uint256 _amount, uint256 _durationSec) public {
        PoolInfo storage pool = poolInfo[_pid];
